...
- Core Power Actions: Perform key tasks (Boot, Reboot, Shutdown) and refresh telemetry directly from the sidebar panel—with a confirmation prompt to prevent accidental misclicks.
...
- 便捷服务器控制：支持直接在面板中执行开机 (Boot)、关机 (Shutdown) 和重启 (Reboot) 指令，并设有操作二次确认，防止误触。
